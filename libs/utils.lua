...
function math.getangleb(x2,x1,y2,y1) 
	return math.atan2(x2-x1,-(y2-y1))
end
function math.getanglevec(a,b) 
...
function math.randomFloat(low, high)
        return low + ((high - low) * random())
end

function randomClamped()
	random()
	--Rightbound + 1 since 0 is counted as leftbound.
	local ran = random(-5,6) 
	...
	--return ran <= 0 and -1 or (ran > 0 and 1)
end
...
local function boxtocircle(b)
	local circle = {}
	circle.x = (b.x + b.w) * 0.5
	circle.y = (b.y + b.h) * 0.5
	circle.r = math.sqrt((b.h^2) + (b.w^2))
	return circle
end

function CircleCircleCollision(a, b)
	local t_radius = a.r+b.r--math.sqrt(a.r + b.r)
