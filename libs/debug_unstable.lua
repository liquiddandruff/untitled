...
_Debug.overlay = function()
	...
	lg.setColor(R, G, B, 220)
	lg.rectangle("fill", 0, 0, w, h)
	lg.setColor(255, 255, 255)
	lg.setFont(_Debug.Font)
	local count = 0
	local cutY = 0
	...
	if #_Debug.Proposals > 0 then
		if #_Debug.Proposal_String > 0 or _Debug.ProposalLocation ~= _G then
			...
			lg.print(_Debug.Proposals[index], 20 + len, h - 27)
			for i = index + 1, (#_Debug.Proposals > 5) and 5 or #_Debug.Proposals do
				lg.setColor(127, 127, 127, 255 - 255/6 * (i - 1))
				lg.print(_Debug.Proposal_String .. _Debug.Proposals[i], x_add + 20, h - 27 - ((fontSize - 1) * (i - 2)))
			end
			if index ~= 1 then
				lg.setColor(127, 127, 127)
				lg.print(_Debug.Proposal_String .. _Debug.Proposals[1], x_add + 20, h - 27 - ((fontSize - 1) * -1))
			end
			lg.setColor(255, 255, 255)
		end
	end
	if _Debug.drawTick then
		lg.print("_", 20 + len, h - 27)
	end
	lg.print(str, 20, h - 27)
	if (#_Debug.order - _Debug.longestOffset > _Debug.lastRows - 1) then
		lg.setFont(_Debug.BigFont)
		lg.print("...", w - 30, h - 30)
	end
	lg.setColor(r, g, b, a)
	lg.setFont(font)
...
_Debug.keyConvert = function(key, unicode)
	...
	if key == "return" then --Execute Script
		print("> " .. _Debug.input)
		if(_Debug.input=="who is the best?") then
			print("steven huang is undoubtely the best")
		end
